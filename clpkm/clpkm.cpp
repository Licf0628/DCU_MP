...
void hexArray2bytes(const uint8_t* hexStr, uint8_t* byteArray, size_t byteArraySize) {		//默认hex为正确的十六进制数组不以‘\0’结尾
...
void bytes2hexArray(const uint8_t* input, uint8_t* output, size_t input_len) {
	...
	CLPKM_COM::CLPKM_COM(std::string& id, const SM2_BN mpri) :ID(id), mPriKey(mpri) {
		...
		printPara("DAi:", DAi);
#endif
	setMKeyPair(mPriKey, mPubKey);		
	printPara("mPriKey:", mPriKey);
	printPara("mPubKey:", mPubKey);
	genKeyPairForKgcSelf();		//kgc生成用户密钥对
	std::cout << "KGC init successful!" << std::endl;
}
int CLPKM_COM::setMKeyPair(Big& mprikey, Epoint& mpubkey) {
	if (!isKgc) throw std::logic_error("KGC calls only!");
	mPriKey = mprikey;
	mPubKey = mpubkey; 
	isGetKgcPubKey = true;
	return 0;
...
SM2_KEY CLPKM_COM::getMKeyPair(void) {
	if (!isKgc) throw std::logic_error("KGC calls only!");
	SM2_KEY pair; 
	mPriKey.toBytes(pair.private_key);
	pair.public_key = mPubKey.getPoint(); 
	return pair; 
}
...
int CLPKM_COM::genKeyPairForKgcSelf(void) {
	if (!isKgc) {
		throw std::logic_error("KGC calls only!");
		return -1;
	}
	...
	MpubKey =getMPubKey();	
	genUA(UA);
	genWA_TA(this->ID,UA, WA, TA);
	if (genDA(TA, DA) != 0) {
		throw std::logic_error("Private key generation failed, key generation process terminated!\n");
		return -2;
	}
	setKeyPair(DA, WA);
	
	if (verifyKeyPair() != 0) {
		throw std::logic_error("Failed verify keyPair\n");
		return -3;
	}
	printPara("KGC user PriKey:", DA);
	printPara("KGC user PubKey:", WA);
	return 0;
}
...
int genWA_TA_C(uint8_t *IDA,int IDAlen,const SM2_KEY*mKeyPair, const SM2_POINT* UA, SM2_POINT* WA, SM2_BN TA) {
	SM2_BN w, lambda, HA;
	uint8_t Wc[32];
#if CLPKM_VERIFY_MODE
	for (int i = 0; i < BNSIZE; i++) w[i] = testW[i];
#else
	sm2_bn_rand_range(w, SM2_N);
#endif
	sm2_bn_to_bytes(w, Wc);
	//WA=[w]G+UA
	sm2_point_mul_generator(WA, Wc);
	sm2_point_add(WA, WA, UA);	

	genHA(IDA,IDAlen,&mKeyPair->public_key,HA);
	genLambda(WA, HA, lambda);

	SM2_BN priKey,tmp;
	sm2_bn_from_bytes(priKey, mKeyPair->private_key);
	sm2_bn_init(TA);
	sm2_fn_add(TA, TA, w);
	sm2_fn_mul(tmp, lambda, priKey);
	sm2_fn_add(TA, tmp, TA);		//TA = (w + lambda * mPriKey) mod n，
	
#if CLPKM_DEBUG_PRINT
	sm2_print_bn("w", w);
	sm2_print_bn("lambda:", lambda);
	sm2_print_bn("HA:", HA);
	hexCharBuf_print("C_WAx:", WA->x, POINT_SIZE);
	hexCharBuf_print("C_WAy:", WA->y, POINT_SIZE);
	sm2_print_bn("TA:", TA);
#endif
	return 0;
}
