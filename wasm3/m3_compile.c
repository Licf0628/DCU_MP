...
static
M3Result Compile_SM2_Verify (IM3Compilation o, m3opcode_t i_opcode)
{
    _try {
        // 输出日志以便调试：显示当前正在编译 SM2_VERIFY 指令
     //  d_m3Log(compile, d_indent "(SM2_VERIFY)");
        // 由于 SM2_VERIFY 指令没有额外立即数，
        // 直接发出对应的操作码（op_sm2_verify）即可。

	_ (EmitOp(o, op_sm2_verify));
	for(int i=0;i<6;i++){
	//printf("TopIndex:%d\n",GetStackTopIndex(o));
        i16 stackTop = GetStackTopIndex (o)-i;
        u16 srcSlot = GetSlotForStackIndex (o, stackTop);
	//printf("TopIndex:%d\n",GetStackTopIndex(o));
        EmitSlotOffset (o, srcSlot);
	//	_           (Pop (o));
...
//	_       (Push (o, c_m3Type_i32, GetStackTopIndex(o)));
...
const M3OpInfo c_operationsFC [] =
{
	...
	M3OP( "sm2.verify",            -5,  i_32,   d_emptyOpList,                           Compile_SM2_Verify ), // 0x0c
