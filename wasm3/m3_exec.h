...
{
// 从操作数栈中获取六个参数 
    u32 resultsOffset= slot(u32);
    u32 size	     = slot(u32);
    u32 len          = slot(u32);
    u32 msgOffset    = slot(u32);
    u32 sigOffset    = slot(u32);
    u32 pubkeyOffset = slot(u32);

    // 检查各个内存访问是否越界
    // 检查内存访问是否越界
    if (pubkeyOffset + SM2_PUBKEY_SIZE*size > _mem->length) {
        d_outOfBoundsMemOp(pubkeyOffset, SM2_PUBKEY_SIZE*size);
        return "Memory out of bounds: pubkey";
    }
    if (sigOffset + SM2_SIG_SIZE*size > _mem->length) {
        d_outOfBoundsMemOp(sigOffset, SM2_SIG_SIZE*size);
        return "Memory out of bounds: sig";
    }
    if (msgOffset + SM2_MSG_SIZE*size > _mem->length) {
        d_outOfBoundsMemOp(msgOffset, SM2_MSG_SIZE * size);
        return "Memory out of bounds: msg";
    }
    if (resultsOffset + SM2_RES_SIZE * size > _mem->length) {
        d_outOfBoundsMemOp(resultsOffset, SM2_RES_SIZE * size);
        return "Memory out of bounds: results";
    }
    // 调用 m3_sm2_verify
    IM3Runtime runtime = m3MemRuntime(_mem);
    int hcsModel = runtime->hcsAcceModel;
    _r0 = m3_sm2_verify( _sp, _mem,pubkeyOffset, sigOffset,
                                    msgOffset, len, size, resultsOffset,(C_DeviceType)hcsModel);
...
d_m3Op (CopySlot_32)
{
//printf("src debug->pc:%x\n",*_pc);
//                printf("_r0_0: %x\n", _r0);
    u32 * dst = slot_ptr (u32);
//printf("dst debug->pc:%x\n",*_pc);
//                printf("_r0_1: %x\n", _r0);
