...
M3Result repl_init(unsigned stack,int hcsModel)
{
    repl_free();
...
int choose_hcs_model(const char* model) { 
    if (!strcmp("CCP",model)) {
        return 1;
    }
    else if (!strcmp("DCU", model)) {
        return 2;
    }
    else if (!strcmp("HCS", model)) {
        return 3;
    }
   return 0;
...
void print_usage() {
    ...
    puts("  --hcs-acce-model      Vector operator acceleration mode  default: CPU(DCU/CCP/HCS)");
}
...
int  main(int i_argc, const char* i_argv[])
{
    ...
    while (i_argc > 0)
    {
        ...
        else if (!strcmp("-hcs-acce-model", arg)) {
            const char *tmp = "CPU";
            ARGV_SET(tmp);
            hcsModel=choose_hcs_model(tmp);
        }
    ...
    result = repl_init(argStackSize,hcsModel);
