#ifndef __SM3_ALG_HIP_H__
#define __SM3_ALG_HIP_H__
...
#ifdef SM3_SSE3
#error("HIP version not surport SM3_SSE3");
...
#ifndef SM3_AVX_BMI2
__device__ void sm3_compress_blocks_hip(uint32_t digest[8], const uint8_t* data, size_t blocks)
{
	uint32_t A;
	uint32_t B;
	uint32_t C;
...
__device__ void sm3_init_hip(SM3_CTX* ctx)
{
...
__device__ void sm3_update_hip(SM3_CTX* ctx, const uint8_t* data, size_t data_len)
{
	size_t blocks;
	...
	if (ctx->num) {
		...
		if (data_len < left) {
			...
			sm3_compress_blocks_hip(ctx->digest, ctx->block, 1);
			ctx->nblocks++;
			data += left;
			data_len -= left;
		}
	...
	if (blocks) {
		sm3_compress_blocks_hip(ctx->digest, data, blocks);
		ctx->nblocks += blocks;
...
__device__ void sm3_finish_hip(SM3_CTX* ctx, uint8_t* digest)
{
	int i;
...
__device__ void sm3_digest_hip(const uint8_t* msg, size_t msglen,
	uint8_t dgst[SM3_DIGEST_SIZE])
{
	SM3_CTX ctx;
	sm3_init_hip(&ctx);
	sm3_update_hip(&ctx, msg, msglen);
	sm3_finish_hip(&ctx, dgst);
}
#endif
