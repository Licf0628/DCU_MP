...
#include <unistd.h>
#include <config.h>
#ifndef __SM2_ALG_HIP__
#ifdef USE_DCU_DEVICE_ACCE
#define __SM2_ALG_HIP__
...
__device__ const unsigned char SM2_a[] = {
	0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC
};
__device__ const unsigned char SM2_b[] = {
	0x28, 0xE9, 0xFA, 0x9E, 0x9D, 0x9F, 0x5E, 0x34,
	0x4D, 0x5A, 0x9E, 0x4B, 0xCF, 0x65, 0x09, 0xA7,
	0xF3, 0x97, 0x89, 0xF5, 0x15, 0xAB, 0x8F, 0x92,
	0xDD, 0xBC, 0xBD, 0x41, 0x4D, 0x94, 0x0E, 0x93
};
__device__ const unsigned char SM2_xg[] = {
	0x32, 0xC4, 0xAE, 0x2C, 0x1F, 0x19, 0x81, 0x19,
	0x5F, 0x99, 0x04, 0x46, 0x6A, 0x39, 0xC9, 0x94,
	0x8F, 0xE3, 0x0B, 0xBF, 0xF2, 0x66, 0x0B, 0xE1,
	0x71, 0x5A, 0x45, 0x89, 0x33, 0x4C, 0x74, 0xC7
};
__device__ const unsigned char SM2_yg[] = {
...
__device__ int sm2_do_sign_hip(const SM2_KEY* key, const uint8_t dgst[32], SM2_SIGNATURE* sig, unsigned int index)
{
	...
	sm2_bn_from_bytes_hip(d, key->private_key);

	sm2_fn_add_hip(d_inv, d, SM2_ONE_HIP);
	if (sm2_bn_is_zero_hip(d_inv)) {
		return -1;
	}
	sm2_fn_inv_hip(d_inv, d_inv);


	sm2_bn_from_bytes_hip(e, dgst);

retry:

	//do {
	//	if (sm2_fn_rand_hip(k, index+1) != 1) {
	//		return -1;
	//	}
	//	
	//} while (sm2_bn_is_zero_hip(k));

	sm2_bn_from_bytes_hip(k, dgst);//这里的随机数生成应该重写
	sm2_jacobian_point_mul_generator_hip(P, k);
	sm2_jacobian_point_get_xy_hip(P, x, NULL);



	if (sm2_bn_cmp_hip(e, SM2_N_HIP) >= 0) {
		sm2_bn_sub_hip(e, e, SM2_N_HIP);
	}
	if (sm2_bn_cmp_hip(x, SM2_N_HIP) >= 0) {
		sm2_bn_sub_hip(x, x, SM2_N_HIP);
	}
	sm2_fn_add_hip(r, e, x);


	sm2_bn_add_hip(t, r, k);
	if (sm2_bn_is_zero_hip(r) || sm2_bn_cmp_hip(t, SM2_N_HIP) == 0) {
		goto retry;
	}


	sm2_fn_mul_hip(t, r, d);		//sm2_bn_print(stderr, 0, 4, "r*d", t);
	sm2_fn_sub_hip(k, k, t);		//sm2_;bn_print(stderr, 0, 4, "k-r*d", k);
	sm2_fn_mul_hip(s, d_inv, k);	//sm2_bn_print(stderr, 0, 4, "s = ((1 + d)^-1 * (k - r * d)) mod n", s);


	if (sm2_bn_is_zero_hip(s)) {
		goto retry;
	}

	sm2_bn_to_bytes_hip(r, sig->r);	//sm2_bn_print_bn(stderr, 0, 4, "r", r);
	sm2_bn_to_bytes_hip(s, sig->s);	//sm2_bn_print_bn(stderr, 0, 4, "s", s);
...
__device__ void sm2_do_verify_hip(const SM2_KEY* key, const uint8_t dgst[32], const SM2_SIGNATURE* sig, uint8_t* result)
{
	SM2_JACOBIAN_POINT _P, * P = &_P;
	...
	SM2_BN t;
	// parse public key
	sm2_jacobian_point_from_bytes_hip(P, (const uint8_t*)&(key->public_key));
	//sm2_jacobian_point_print(stderr, 0, 4, "P", P);

// parse signature values
	sm2_bn_from_bytes_hip(r, sig->r);	//sm2_bn_print(stderr, 0, 4, "r", r);
	sm2_bn_from_bytes_hip(s, sig->s);	//sm2_bn_print(stderr, 0, 4, "s", s);

	// check r, s in [1, n-1]
	if (sm2_bn_is_zero_hip(r) == 1
		|| sm2_bn_cmp_hip(r, SM2_N_HIP) >= 0
		|| sm2_bn_is_zero_hip(s) == 1
		|| sm2_bn_cmp_hip(s, SM2_N_HIP) >= 0) {
		*result = 9;
		return;
	}

	// e = H(M)
	sm2_bn_from_bytes_hip(e, dgst);	//sm2_bn_print(stderr, 0, 4, "e = H(M)", e);

	// t = r + s (mod n), check t != 0
	sm2_fn_add_hip(t, r, s);		//sm2_bn_print(stderr, 0, 4, "t = r + s (mod n)", t);
	if (sm2_bn_is_zero_hip(t)) {
		*result = 10;
		return;
	}

	// Q = s * G + t * P
	sm2_jacobian_point_mul_sum_hip(R, t, P, s);
	sm2_jacobian_point_get_xy_hip(R, x, NULL);
...
// r' = e + x (mod n)
	...
	sm2_fn_add_hip(e, e, x);		//sm2_bn_print(stderr, 0, 4, "e + x (mod n)", e);

	// check if r == r'
	if (sm2_bn_cmp_hip(e, r) != 0) {
		*result = 11;
		return;
	}
	*result = 1;
